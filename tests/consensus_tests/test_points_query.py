...
from .utils import (
    assert_project_root, make_peer_folders, start_first_peer, wait_peer_added,
    wait_for_uniform_cluster_status, wait_collection_exists_and_active_on_all_peers,
    wait_collection_points_count, start_peer
)
...
def test_deduplication_and_sorting_with_consistency(tmp_path: pathlib.Path, consistency):
    assert_project_root()
    peer_dirs = make_peer_folders(tmp_path, N_PEERS)
    (bootstrap_api_uri, bootstrap_uri) = start_first_peer(peer_dirs[0], "dedup_peer_0_0.log")
    peer_api_uris = [bootstrap_api_uri]
    leader = wait_peer_added(bootstrap_api_uri)
    for i in range(1, N_PEERS):
        peer_api_uris.append(start_peer(peer_dirs[i], f"dedup_peer_0_{i}.log", bootstrap_uri))
    ...
    for i, vec in enumerate(vectors):
        r = requests.put(
            f"{peer_api_uris[i]}/collections/test_collection/points?wait=true", json={
                "points": [{"id": point_id, "vector": vec}]
            })
        assert_http_ok(r)
    ...
    for uri in peer_api_uris:
        ...
        assert len(ids) == len(set(ids)), f"Duplicate IDs found in results with consistency={consistency}"
        # Assert sorted by score descending
        scores = [point["score"] for point in results]
        assert scores == sorted(scores, reverse=True), f"Results not sorted by score with consistency={consistency}"
